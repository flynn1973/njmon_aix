...
#define NONZERO(x) ((x)?(x):1)
		...
		pdouble("wserv_avg", (double)(HWTICS2MSECS(DISK_DELTA(wserv)) / 
							NONZERO( DISK_DELTA(xfers) - DISK_DELTA(__rxfers))));
...
int ret;
	...
	DEBUG printf("no file or can't open it\n");
	return; /* no file */
    }
    DEBUG printf("file descriptor=%d\n",fd);
    DEBUG printf("file exists and readable and opened\n");
    if(read(fd, buffer, 31) > 0) { /* has some data */
	    DEBUG printf("file has some content\n");
	    buffer[31]=0;
	    if( sscanf(buffer, "%ld", &pid) == 1) {
		DEBUG printf("read a pid from the file OK = %ld\n",pid);
		ret = kill(pid, 0);
		DEBUG printf("kill %ld, 0) = returned =%d\n",pid, ret);
		if(ret == 0) {
			DEBUG printf("we have a njmon running - exit\n");
			exit(13);
		}
	    }
    }
